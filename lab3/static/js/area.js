...
function area(data) {
  ...
  //Creates the big chart
    var area = d3.area()
        ...
        .x(function (d) { /*console.log(d.time.substring(0,10));*/return x(d.time); })
        .y0(height)
  ...
  y.domain([2, d3.max(data, function (d) { return d.mag; })]);
  x2.domain(x.domain());
  y2.domain(y.domain());


  //Defines the focus area = large area?
  var focus = svg.append("g")
          .attr("transform", "translate(" + margin.left + "," + margin.top + ")");

  //Defines the context area = small area?
